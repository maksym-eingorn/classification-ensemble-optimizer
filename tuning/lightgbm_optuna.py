...
def run_optuna_kfold_lightgbm(
    ...
    """
    Run Optuna to tune a LightGBM classifier with stratified K-fold
    cross-validation.

    Out-of-fold probabilities are stored for future ensemble search. Each
    Optuna trial trains one LightGBM configuration across all K folds and
    ...
    study.optimize(objective_lgb, n_trials=n_trials, n_jobs=n_jobs)
